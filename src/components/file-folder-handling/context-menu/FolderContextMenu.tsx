...
interface FolderContextMenuProps {
  contextMenu: {
    x: number;
    y: number;
    type: 'empty' | 'folder' | 'trash-folder';
    folderId?: string;
    folderName?: string;
  };
  ...
  onAddDocument: () => void;
  onRenameFolder: () => void;
  onMakePublicFolder: () => void;
  onDeleteFolder: () => void;
  onRestoreFolder: () => void;
  onPermanentlyDeleteFolder: () => void;
  onExportFolder: () => void;
  onClose: () => void;
  isClosing?: boolean;
}
    ...
    return (
      <>
        ...
        >
          ...
          <div className="pb-safe-area-inset-bottom">
            {(contextMenu.type === 'empty' || contextMenu.type === 'folder') && (
              <>
                <button
            ...
            {contextMenu.type === 'folder' && (
              <>
                ...
                <button
                  onClick={() => { onMakePublicFolder(); onClose(); }}
            ...
            {contextMenu.type === 'trash-folder' && (
              <>
                <button
                  onClick={() => { onRestoreFolder(); onClose(); }}
                ...
                <button
                  onClick={() => { onPermanentlyDeleteFolder(); onClose(); }}
                  className="w-full px-6 py-4 text-left text-red-300 active:bg-red-500/20 transition-colors flex items-center gap-3 border-t border-white/10"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
      ...
      >
        ...
        {contextMenu.type === 'trash-folder' && (
          <>
            ...
            <button
              onClick={onPermanentlyDeleteFolder}
              className="w-full px-4 py-3 text-left text-red-300 hover:bg-red-500/20 active:bg-red-500/30 hover:text-white transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
